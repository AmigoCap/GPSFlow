...
def fullSpeedSegmentation(lSegments):
	for segment_mouvement in lSegments:
		if len(segment_mouvement['velocity']) > 3:
			(lK, whitened) = applyKMeans(segment_mouvement,
												k=int(len(segment_mouvement['velocity']) / 10) + 1)

			lBoundiaries = getBoundiaries(lK)
			lFirstSpeedSegmentation = calcFirstSegmentation(lBoundiaries, whitened, bPadd=False)
			lFirstSpeedSegmentation = cancelWhithen(lFirstSpeedSegmentation, segment_mouvement)
			(speedAgglomerates, a) = agglomerateSpeedSegments(lFirstSpeedSegmentation,
																	  lowThreshold=8,
																	  highThreshold=40,
																	  bMedian=False)
			offset = 0
